...
def _edit_rule(path: str) -> None:
    opr.wipe(DEBUG)
    opr.print_from("Filesorter - Wizard | Edit Rule", f"✏️  Editing the rules for {path}...")
    opr.print_from("Filesorter - Wizard | Edit Rule", f"📂 These are the directories that the matching files will be moved to", after_return_count=1)
